...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "INSERT INTO doctor (doctor_id, doctor_name, specialization, contact_info) 
            VALUES (doctor_seq.NEXTVAL, :doctor_name, :specialization, :contact_info)";
    $stmt = oci_parse($conn, $sql);
    oci_bind_by_name($stmt, ":doctor_name", $doctor_name);
    oci_bind_by_name($stmt, ":specialization", $specialization);
    oci_bind_by_name($stmt, ":contact_info", $contact_info);
    
    if (oci_execute($stmt)) {
        $success_msg = 'Doctor registered successfully!';
    } else {
...
<head>
    ...
    <title>Add Doctor - AidNexus</title>
            ...
            <form method="POST">
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="specialization" placeholder="e.g., General Physician, Psychiatrist" required>
                    </div>
                </div>
                ...
                <button type="button" class="submit-btn" style="background-color: var(--text-light); margin-left: 10px;" onclick="window.location.href='all_doctors.php'">Cancel</button>
            </form>
        </div>
    </div>
</body>
