...
// Fetch all victims
$victims = [];
$sql = "SELECT victim_id, victim_name, dob, gender, contact_info, address FROM victim ORDER BY victim_id DESC";
...
<head>
    ...
    <title>All Victims - AidNexus</title>
            ...
            <table>
                ...
                <tbody>
                    <?php if (empty($victims)): ?>
                        <tr><td colspan="6" style="text-align: center; padding: 40px; color: var(--text-light);">No victims registered yet.</td></tr>
                    <?php else: ?>
                        <?php foreach ($victims as $victim): ?>
                            <tr>
                                <td><?php echo $victim['VICTIM_ID']; ?></td>
                                <td><?php echo htmlspecialchars($victim['VICTIM_NAME']); ?></td>
                                <td><?php echo format_date($victim['DOB']); ?></td>
                                <td><?php echo htmlspecialchars($victim['GENDER']); ?></td>
                                <td><?php echo htmlspecialchars($victim['CONTACT_INFO']); ?></td>
                                <td><?php echo htmlspecialchars($victim['ADDRESS']); ?></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
